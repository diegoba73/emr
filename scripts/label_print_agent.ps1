...
if (Test-Path -LiteralPath $ConfigPath) {
    try {
        ...
        Write-Warning "No se pudo leer config. Se usan defaults. $_"
    }
}

$CsPath = Join-Path $ScriptDir 'EmrRawPrinter.cs'
if (-not (Test-Path -LiteralPath $CsPath)) {
    throw "Falta EmrRawPrinter.cs en $ScriptDir"
}
...
function Resolve-LabelPrinterName {
    ...
    $all = Get-InstalledPrinterNames
    $match = $all | Where-Object { $_ -match '3nStar|LDT114|ZDesigner|4BARCODE|4B-2054' } | Select-Object -First 1

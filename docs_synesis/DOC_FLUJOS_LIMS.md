...
- **Auditoría (B2 / B2-A):** `log_update` por resultado con metadata: `resultado_id`, `solicitud_id`, `numero_solicitud`, `muestra_id`, `valor_presente`, `muestra_anterior_id` / `muestra_nueva_id` si cambió la asociación; **sin `codigo_barra`** ni valores clínicos en metadata. `before_state` / `after_state` de `ResultadoExamen` redactan campos de valor vía `safe_model_snapshot` (`<valor clínico redactado>`). El `codigo_barra` de la muestra sigue disponible en API operativa, no en `AuditEvent`. Transición de orden: metadata en `laboratorio/solicitud_estado.py`.

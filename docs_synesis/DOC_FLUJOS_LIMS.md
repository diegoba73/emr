...
- **Transición automática `RECIBIDA` → `EN_PROCESO` (Fase B2.1):** al cargar un resultado asociando una muestra `RECIBIDA`, el servicio `aplicar_iniciar_proceso` (en `laboratorio/muestra_estado.py`) la pasa a `EN_PROCESO` dentro de la misma transacción atómica, con `select_for_update()` sobre la fila, generando `EventoMuestra` (`accion=EN_PROCESO`) y `AuditEvent UPDATE`. La transición es **idempotente** (si ya está `EN_PROCESO`, no genera evento adicional). Si la muestra está en otro estado, se omite silenciosamente (la rama de carga ya rechazó el caso). La metadata del `log_update` del resultado incluye `muestra_estado_anterior="RECIBIDA"` y `muestra_estado_nuevo="EN_PROCESO"` cuando la transición se efectivizó.

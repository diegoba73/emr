...
- **PERM-FE-LAB-01 (jun 2026):** `permissions.test.ts` — matriz `laboratorio + is_staff` (EMR bloqueado, LIMS permitido, staff no-lab preservado); `Sidebar.test.tsx` — sin enlaces Pacientes/Consultas/Auditoría, con Órdenes LIMS/Microbiología; `turnoPermissions.ts` alineado con `isEmrStaffOrAdmin`.
- **QA-SEED-01 (jun 2026):** `core/management/commands/seed_data.py` — usuarios `enfermeria1`/`secretaria1`; pacientes demo Uno/Ajeno; turno+atención QA; orden LIMS `LAB-DEMO-QA-00001` + muestra + resultado demo. Idempotente (`python manage.py seed_data` ×2). Documentado en `docs/dev-start.md`, `DOC_FLUJOS_EMR.md`, `DOC_FLUJOS_LIMS.md`.
- **Internación tests (jun 2026):** `internacion/tests/helpers.py` (`unique_suffix`, `unique_cie10_code` ≤10 chars); admisión crea internación activa sobre cama `DISPONIBLE` (modelo la marca `OCUPADA`); infraestructura usa `sector_id` en POST camas y nombres únicos por corrida.

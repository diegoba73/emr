...
- **QA-FE-ERR-03 (jun 2026):** `no-raw-lims-ui-errors-guard.test.ts` — `components/lims/*` y `pages/laboratorio/*` sin `toast.error(formatDrfError(...))` ni `response.data` en feedback visible; toasts LIMS usan `getSafeClinicalActionMessage`. `apiError.test.ts` extendido (acciones LIMS adicionales). Deuda remanente: `formatLimsPdfDownloadError` en descarga PDF puede parsear `detail` DRF (helper separado, fuera de alcance toast).

...
**B3-frontend-validación-A (jun 2026) [IMPLEMENTADO]** — **Bloqueo operación técnica en estados cerrados:** `ESTADOS_BLOQUEAN_OPERACION_MICRO = {CANCELADO, VALIDADO, INFORMADO}` en servicio (`microbiologia_estado.py`; sin cambiar modelo ni permisos). `assert_estudio_micro_operable` en todas las mutaciones técnicas + guards PATCH en `views_microbiologia.py`. Excepción: `marcar-informado` desde `VALIDADO`. UI (`MicrobiologiaEstudioDetalle`, `limsAccess.ts`) alinea formularios; datos históricos visibles. Tests: `TestEstudioMicroCerradoOperacionAPI`, `limsAccess.test.ts`.

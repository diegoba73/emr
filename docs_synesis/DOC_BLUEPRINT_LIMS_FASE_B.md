...
**Estado de fases B implementadas:** B0/B1 (catálogos + `Muestra` transaccional), **B2** (vínculo `ResultadoExamen.muestra`, `cargar-resultados` con `muestra_id`, `PROCESAMIENTO`) y **B2-A** (docs + `safe_model_snapshot` sin PHI en snapshots/metadata genéricos de resultado) cerradas. **B2.1:** `validar` con `select_for_update`; transición RECIBIDA/CONSERVADA→EN_PROCESO al cargar primer resultado. **B2-B:** `TipoExamen.requiere_muestra` (default `False`); validación centralizada en `resultado_muestra_validacion.assert_tipo_examen_muestra_carga`; tipos configurados exigen `muestra_id` en carga; coincidencia con `tipo_muestra_requerida` del catálogo; legacy preservado para tipos no configurados. Frontend LIMS pendiente para selección de muestra.

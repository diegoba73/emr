...
> **[HISTÓRICO — blueprint Fase B / mayo 2026]** este archivo es diseño incremental. **[VIGENTE sep 2026]** máquina y permisos de **orden** (`SolicitudExamen`): `DOC_ESTADOS_TRANSICIONES.md`, `DOC_FLUJOS_LIMS.md`, `DOC_PERMISOS_AUDITORIA.md`. No usar las frases “endpoints vigentes … cancelar / marcar-entregado” ni “validar solo admin” de las secciones 1.2–1.3 como contrato actual.
...
**[HISTÓRICO]** el diseño Fase A (mayo 2026) auditaba transiciones vía `apply_solicitud_estado_transition` + `log_update`. **[VIGENTE]** choices y transiciones actuales: `DOC_ESTADOS_TRANSICIONES.md` / `DOC_FLUJOS_LIMS.md`. Endpoints de orden actuales: `DOC_API_ENDPOINTS.md` (`tomar-muestra`, `cargar-resultados`, `validar` / `finalizar`). **No** hay `cancelar` ni `marcar-entregado` de orden.

### 1.3 Permisos (`api/permissions.py`)

**[VIGENTE]** `LimsSolicitudExamenPermission`: escritura `ROLES_LIMS_WRITE` (`admin`, `laboratorio`, `bioquimico`); `validar`/`finalizar` = `ROLES_LIMS_VALIDAR` (`admin`, `bioquimico`) + superuser. **`laboratorio` no valida.** **[HISTÓRICO]** “validar solo admin” y “laboratorio cancela / marca entregado”.
...
### 6.3 Coordinación `SolicitudExamen` (Fase A + B) — **[HISTÓRICO / diseño]**

Las preguntas de esta tabla usan nombres de orden Fase A (`TOMA_MUESTRA`, `CANCELADO`) y **no** son el contrato vigente de `SolicitudExamen` (sep 2026). Máquina actual: `DOC_ESTADOS_TRANSICIONES.md`.
...
### 8.1 Mantener (sin cambiar contrato) — Fase A — **[HISTÓRICO]**

Lista de entonces en el blueprint: GET/POST solicitudes, PATCH sin estado, `tomar-muestra`, `cargar-resultados`, `validar`, `marcar-entregado`, `cancelar`, `etiqueta`. **[VIGENTE]** `DOC_API_ENDPOINTS.md`: no hay `marcar-entregado` ni `cancelar` de orden; alias `finalizar` de `validar`.

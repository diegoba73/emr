...
**Estado de fases B implementadas:** B0/B1 (catálogos + `Muestra` transaccional) y **B2** (`ResultadoExamen.muestra` FK opcional, `cargar-resultados` con `muestra_id`, estados RECIBIDA/CONSERVADA/EN_PROCESO, `EventoMuestra.PROCESAMIENTO`, auditoría sin `codigo_barra`/valor clínico, no rechazar muestra con resultados) cerradas. **B2.1:** `validar` con `select_for_update` en muestras referenciadas; transición RECIBIDA/CONSERVADA→EN_PROCESO al cargar primer resultado.

...
- **Fase B2-B — obligatoriedad progresiva:** `TipoExamen.requiere_muestra` (default `False`; activación por admin/DB en esta fase). Si `requiere_muestra=True`, `POST …/cargar-resultados/` exige `muestra_id` efectivo (payload o FK previa); sin muestra → **400** (`"Este tipo de examen requiere una muestra asociada."`). **B2-B-A:** si hay muestra asociada (aunque `requiere_muestra=False`), `muestra.tipo_muestra` debe coincidir con `tipo_muestra_requerida` del catálogo; no se permite asociar tipo físico incorrecto “porque no exige muestra”. Sin `muestra_id` y `requiere_muestra=False` → carga legacy OK. Fallos no crean resultado/auditoría de éxito ni `EventoMuestra.PROCESAMIENTO`.
- **Fase B2-C — UI LIMS:** el frontend refleja `requiere_muestra` (badge + bloqueo de submit) pero **no sustituye** al backend; catálogo `TipoExamen` vía API read-only; activación de `requiere_muestra` sigue en admin/DB.
...
**Implementado (Fase B3.1 — microbiología base):** modelos `MedioCultivo`, `EstudioMicrobiologia`, `SiembraMicrobiologia`, `LecturaCultivo`; flujo Solicitud → Muestra (`RECIBIDA`/`CONSERVADA`/`EN_PROCESO`) → Estudio → Siembra → Lectura; estados cableados del estudio (`PENDIENTE`/`RECIBIDO`/`SEMBRADO`/`LECTURA_PRELIMINAR`/`CANCELADO`); cambios de estado solo por acciones (`iniciar`, `cancelar`) o transiciones automáticas en creación de siembra/lectura; cancelación con motivo obligatorio; medios se desactivan (no se borran); microbiología **no** se serializa en `ResultadoExamen.valor_obtenido`; migración **`0005_lims_b3_1_microbiologia_base`**. **B3.1-gap [IMPLEMENTADO]:** iniciar estudio/siembra con muestra `CONSERVADA` (constante `MUESTRA_ESTADOS_VALIDOS_INICIAR_MICRO`).

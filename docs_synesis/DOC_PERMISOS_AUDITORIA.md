...
- **Sin auto-envío LIMS** en `perform_create` ni en `Solicitud.save()` (DOC-01 / PERM-01-b). La variable histórica `LIMS_AUTO_SEND` **no tiene efecto** en el modelo (legacy). Envío externo **solo** vía `enviar_lims`/`sincronizar_lims` (admin/superuser). Metadata técnica (`destino=lims_externo`, `success`, `paneles_count`, `tipos_count`, `lims_id_presente`; sin PHI ni payload externo).
...
- **`LimsMuestraTransaccionalPermission`** (`MuestraTransaccionalViewSet`): list/retrieve/eventos `admin` / operadores `ROLES_LIMS_WRITE` (`laboratorio`, `bioquimico`) / `medico`; create/update/partial y acciones `tomar`/`recibir`/`rechazar`/`conservar`/`descartar`/`cambiar-ubicacion`/`cancelar`/`tomar_por_codigo`/`recibir_por_codigo` solo **`ROLES_LIMS_WRITE`** (admin + laboratorio + bioquímico) + superuser; **`etiqueta_zpl`** (preview JSON+ZPL 40×23), **`imprimir_etiqueta`** (prepara ZPL para agente USB local) y **`confirmar_impresion_etiqueta`** (audit `transport=local_agent`): mismo **`ROLES_LIMS_WRITE`** (sin médico); PDF legacy `etiqueta` sigue en allowlist con lectura médica a nivel `has_permission` (object permission de médico no opera impresión); `medico` solo lectura de muestras cuya solicitud tiene `medico_interno.user` = usuario actual; **paciente** y **secretaría** sin acceso a este endpoint; anónimo denegado; `destroy` denegado en permiso.
- **Auditoría muestra (B1):** `muestra_create`, `muestra_tomar`, `muestra_recibir`, `muestra_rechazar`, `muestra_conservar`, `muestra_descartar`, `muestra_cambiar_ubicacion`, `muestra_update` (PATCH administrativo) y **`muestra_etiqueta_print`** (impresión física ZPL) en metadata `accion`; **sin `codigo_barra`**, sin PHI, sin ZPL, sin `lugar_extraccion` en metadata/snapshots genéricos. Texto completo de `motivo_rechazo`/`observaciones` omitido en snapshots (`auditoria/snapshot.py`); `lugar_extraccion` también redactado en `safe_model_snapshot(Muestra)`.

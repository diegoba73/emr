...
class Command(BaseCommand):
    help = (
        "Importa procedimientos al catálogo simplificado. "
        "Cada fila debe contener el nombre en la primera columna."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "csv_file",
            type=str,
            help="Ruta absoluta o relativa al archivo de procedimientos.",
        )
        parser.add_argument(
            "--encoding",
            default="latin-1",
        ...
        parser.add_argument(
            "--overwrite",
            action="store_true",
            help="Actualiza la descripción si ya existe el procedimiento.",
        )
    ...
    def handle(self, *args, **options):
        ...
        with csv_path.open(newline="", encoding=encoding) as csvfile:
            ...
            for indice, row in enumerate(reader, start=1):
                ...
                try:
                    procedimiento = ProcedimientoCatalogo.objects.filter(nombre__iexact=nombre).first()
                    if not procedimiento:
                        if dry_run:
                            creados += 1
                        else:
                            ProcedimientoCatalogo.objects.create(
                                nombre=nombre,
                    ...
                    cambios = False
                    if overwrite and descripcion and procedimiento.descripcion != descripcion:
                        cambios = True
                        if not dry_run:
                            procedimiento.descripcion = descripcion

                    if not procedimiento.activo:
                        cambios = True
                        if not dry_run:
                    ...
                    if cambios:
                        if dry_run:
                            ...
                            procedimiento.save(update_fields=["descripcion", "activo"])
                            actualizados += 1
                    else:
                        omitidos += 1
        ...
        self.stdout.write(self.style.SUCCESS("Importación de procedimientos completada."))
